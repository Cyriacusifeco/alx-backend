...
This class inherits from BaseCaching and implements a caching system using the Least Recently Used (LRU) algorithm.
"""
...
class LRUCache(BaseCaching):
	...
	"""
	    ...
	    # Update the LRU list with the most recently used key (move it to the end)
            self.lru_list.append(key)

	    # Check if the number of items in the cache exceeds the maximum limit
            if len(self.cache_data) > BaseCaching.MAX_ITEMS:
                if self.lru_list:
		    # Retrieve the least recently used item (LRU) from the beginning of the list
